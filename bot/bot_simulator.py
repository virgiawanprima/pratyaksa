...
app = FastAPI(
    title="PRATYAKSA Bot Simulator API", 
    description="API untuk menerima webhook/alert dan mengirimnya langsung ke Telegram (Tanpa Redis)",
    version="2.0"
)
...
# ── IN-MEMORY CACHE (Pengganti Redis) ─────────────────────────────────────────
fleet_data = {}
...
# ── CORE LOGIC: KIRIM KE TELEGRAM ─────────────────────────────────────────────
async def send_to_telegram(d: dict):
    message = (
...
# ── API ENDPOINTS ─────────────────────────────────────────────────────────────
@app.post("/api/v1/send-alert", summary="Simulasi Tembak Alert ke Bot")
async def trigger_bot_alert(payload: AlertPayload, background_tasks: BackgroundTasks):
    if BOT_TOKEN == "ISI_TOKEN_BOT_ANDA_DI_SINI":
        raise HTTPException(status_code=500, detail="Token Telegram belum dikonfigurasi pada bot_simulator.py")
    ...
    # Jalankan pengiriman Telegram di background agar HTTP response tidak tertunda
    background_tasks.add_task(send_to_telegram, data_dict)
...
@app.get("/api/v1/fleet-status", summary="Cek Status In-Memory Fleet")
